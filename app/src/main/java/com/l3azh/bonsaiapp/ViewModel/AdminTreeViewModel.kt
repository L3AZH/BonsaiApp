...
import com.l3azh.bonsaiapp.Api.Response.TreeResponseData
...
@HiltViewModel
class AdminTreeViewModel @Inject constructor(private val treeRepository: TreeRepository) :
    ViewModel() {
    ...
    fun initData(context: Context) = CoroutineScope(Dispatchers.IO).launch {
        ...
        treeRepository.getAllTree(
            context = context,
            onSuccess = { getAllTreeResponse ->
                CoroutineScope(Dispatchers.Main).launch {
                    state.value.isLoading.value = false
                    state.value.onError.value = false
                    if (getAllTreeResponse.data.isEmpty()) {
                        state.value.isEmpty.value = true
                    } else {
                        state.value.isEmpty.value = false
                        val transform: (TreeResponseData) -> TreeState = { it ->
                            TreeState(
                                it.uuid,
                                it.name,
                                it.description,
                        ...
                        state.value.listTree.value = getAllTreeResponse.data.map(transform)
                    }
                }
            },
